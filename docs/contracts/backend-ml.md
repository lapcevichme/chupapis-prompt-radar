...
> ⚠ **`source_id` проставляется в КАЖДОЙ записи лога**, не только на уровне батча: ml_service читает
> `item.source_id` (у `LogBatch` нет верхнеуровневого `source_id`). Backend `MlClient` инжектит
> `source_id` в каждый лог — иначе атрибуция источника (фильтры дашборда) сломается.
