...
Knowledge & Communications (2080 записей), Engineering & Data (1386), Operations & Planning (1394).
Они делят датасет по категориям без пересечений и проходят **тот же самый ingestion-путь**, что
пользовательская загрузка, — в БД нет захардкоженной аналитики.
