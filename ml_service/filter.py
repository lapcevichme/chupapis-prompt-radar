''''
Это файл которые не удалось реализовать.
Фильтр для OpenWebUI, который автоматически перехватывает запросы пользователей и пытается кинуть на бэкенд
Но есть какие-то непонятные технические шоколадки, из-за которых он не работает.
contact @mihlap on telegram
'''
